...
- **侧边栏标题截断约定**：英文标题的两个点提示不得使用渐变或额外 padding 作为遮罩；常态下用与论文行一致的背景色从截断点覆盖到论文行右侧，并将 `..` 放在遮盖层左侧，避免最后一个可见字母贴到截断点或右侧残留透明区；`1/2/3/4` 按钮 hover/focus 显示时必须隐藏两个点，避免按钮左侧出现残留小点或覆盖块。
